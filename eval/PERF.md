...
### Host e2e: E2B (Metal) + mock-Fireworks — Fireworks çağrı düşüşü

| Set | Önce (FW-only) | Sonra (lokal katman) | Lokal 0-token cevap |
|---|---|---|---|
| tasks.json (64) | 59 çağrı | **23 çağrı** | 38 |
| hard.json (24) | 20 çağrı | **7 çağrı** | 13 |
| components.json (22) | 10 çağrı | **5 çağrı** | 5 |
| practice.json (8) | 7 çağrı | **3 çağrı** | 4 |

Süre: 64 görev ≈ 50 sn (host Metal; grading kutusu CPU'da daha yavaş —
Pacer ModeFull dışında lokali atlıyor). Kalan remote çağrıların büyük kısmı
örneksiz codegen/debug (kanıt üretilemiyor → tasarım gereği güçlü modele).

### E2B doğruluk spot-check'leri ve ölçüme dayalı iki düzeltme

**İyi:** math-PAL 6/6 birebir; factual/NER/summary format-temiz; sarcasm
sentiment (sh-1) doğru; base sentiment 3/3.

**Ölçülen iki lokal hata modu ve düzeltmeleri (TUT):**
1. Logic'te 'Answer:' satırına ulaşamayan lokal cevap = yarım akıl-yürütme
   dökümü → judge'ı geçemez. Düzeltme: lokal logic yalnızca ≤100 karakter
   tek-satır sonuçsa gemiye biner; gevezeleyen güçlü modele gider.
   (logic-2 ayrıca knights çözücüsüne 'We are both knaves' + sessiz katılımcı
   toplama eklenince tamamen 0 token'a düştü.)
2. Sentiment nüans vakaları: kontrastlı-baskın-hüküm (sh-2: Mixed dedi,
   Positive-overall bekleniyor) ve olgusal-aktarım-Neutral (sh-3: Negative
   dedi). İkisi de sözcük işaretli (but/however, describes/reports) →
   `reSentimentNuance` eşleşen sentiment lokali atlar. Base sette maliyet:
   +4 remote çağrı (~200 token) — 2 muhtemel gate hatasına karşı ucuz sigorta.

mh-1'in '222'si mock'un canned PAL ifadesi (canlıda anlamlı), lokal hata değil.
